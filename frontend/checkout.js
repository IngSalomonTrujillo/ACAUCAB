...
async function onPlaceOrder(e) {
    ...
    // Mapear métodos de pago a sus IDs reales
    const metodosPagoIds = pagos.map(p => {
        // Buscar el método de pago en checkoutState.metodosPago por tipo y detalles
        // Si tienes una estructura diferente, ajusta aquí
        let metodo = null;
        if (Array.isArray(checkoutState.metodosPago)) {
            metodo = checkoutState.metodosPago.find(m => m.tipo === p.tipo);
        } else if (typeof checkoutState.metodosPago === 'object') {
            // Si es un objeto, buscar por tipo
            metodo = Object.values(checkoutState.metodosPago).find(m => m.tipo === p.tipo);
        }
        return { id: metodo ? metodo.id || metodo.metodo_pago_id : 1 }; // Fallback a 1 si no se encuentra
    });
    const compraPayload = {
        ...
        metodosPago: metodosPagoIds,
        puntosUsados: checkoutState.puntosUsados,
